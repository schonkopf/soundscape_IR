...
class batch_processing:
    ...
    def params_spectrogram(self, channel=1, offset_read=0, FFT_size=512, environment='wat', time_resolution=None, window_overlap=0, f_range=[], prewhiten_percent=0, padding=0, folder_combine=False, mel_comp=0, sensitivity=0, annotation_iteration=False):
        """
        ...
        self.annotation_iteration=annotation_iteration
    ...
    def run(self, start=1, num_file=None):
            ...
            if self.run_spectrogram:
                if self.cloud==2:
                    ...
                    if self.Raven_selections:
                        ...
                        self.selections_filename = temp2['title']
                    self.path='.'
                ...
                if self.Raven_selections:
                    self.selections_filename = self.Raven_folder + '/' + self.audioname[file][:-4]+self.Raven_selections

                filename_label=self.audioname[file][:-4]
                if self.annotation_iteration:
                    df=pd.read_table(self.selections_filename, index_col=0)
                    df.sort_values(by=['Begin Time (s)'],inplace=True)
                    idx_st = df['Begin Time (s)']
                    idx_et = df['End Time (s)']-df['Begin Time (s)']
                    for iter in np.arange(idx_st.shape[0]):
                        audio = audio_visualization(self.audioname[file], path=self.path, channel=self.channel, FFT_size=self.FFT_size,
                                        ...
                                        offset_read=idx_st.iloc[iter], duration_read=idx_et.iloc[iter], mel_comp=self.mel_comp)
                        if iter==0:
                            self.run_audio(audio, filename_label+'.'+str(self.annotation_iteration+iter+1)[1:], file)
                        else:
                            self.run_audio(audio, filename_label+'.'+str(self.annotation_iteration+iter+1)[1:], file+0.1)
                else:
                    if self.Raven_selections:
                        audio = audio_visualization(self.audioname[file], path=self.path, channel=self.channel, FFT_size=self.FFT_size,
                                        time_resolution=self.time_resolution, window_overlap=self.window_overlap,
                                        f_range = self.f_range, sensitivity=self.sensitivity,
                                        environment=self.environment, plot_type=None, prewhiten_percent=self.prewhiten_percent,
                                        annotation = self.selections_filename, padding = self.annotation_padding, mel_comp=self.mel_comp)
                    else:
                        audio = audio_visualization(self.audioname[file], path=self.path, channel=self.channel, FFT_size=self.FFT_size,
                                        time_resolution=self.time_resolution, window_overlap=self.window_overlap,
                    ...
                    self.run_audio(audio, filename_label, file)
        ...
        if self.run_lts:
            if self.lts_folder_id:
                self.lts.save_lts(self.lts_filename, self.lts_folder_id, status_print=False)
            else:
                self.lts.save_lts(self.save_lts_path+'/'+self.lts_filename, status_print=False)

    def run_audio(self, audio, filename_label, file):
        ...
        if self.run_separation:
            ...
            if self.save_basis:
                filename=filename_label+'.mat'
                if len(self.save_basis_idx)==1:
                    model.W=model.W[:,self.save_basis_idx[0]:self.save_basis_idx[0]+1]
                else:
                    model.W=model.W[:,self.save_basis_idx]
                model.W_cluster=model.W_cluster[self.save_basis_idx]
                model.basis_num=model.W.shape[1]
                model.source_num=len(np.unique(model.W_cluster))
            ...
            if self.run_lts:
                if file==self.start:
                    self.lts = lts_maker(time_resolution=self.lts_time_resolution)
                    self.lts.filename_check(self.dateformat, self.initial, self.year_initial, filename_label)
                    self.lts.f=audio.f
                    self.lts.sf=audio.sf
                    self.lts.link=[]
                self.lts.get_file_time(filename_label)
                self.lts.compress_spectrogram(10**(model.separation[self.lts_source-1][:,1:]/10),
            ...
            if self.run_detection:
                for n in range(0, len(self.source)):
                    filename=filename_label+'_S'+str(self.source[n])+'.txt'
                    ...
                    if self.save_clip_path:
                        if sp.detection.shape[0]>0:
                            audio = audio_visualization(filename_label+'.wav', path=self.path, channel=self.channel, sensitivity=self.sensitivity,
                                           environment=self.environment, plot_type=None,
                                           annotation = self.detection_path+'/'+filename, save_clip_path=self.save_clip_path)

        if self.run_tonal:
            if self.run_separation:
                filename=filename_label+'_S'+str(self.tonal_source[n])+'.txt'
        ...
        if self.run_feature_extraction:
            if self.run_separation:
                filename=filename_label+'_S'+str(self.feature_source)+'.mat'
            ...
            if self.Raven_selections:
                df=pd.read_table(self.selections_filename, index_col=0)
                sp.detection=np.array(df[['Begin Time (s)', 'End Time (s)']])
